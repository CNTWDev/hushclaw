...
class TestRetentionExecutor(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_retention_deletes_files_from_disk(self):
        """RetentionExecutor must unlink artifact files from disk before deleting rows."""
        from hushclaw.memory.store import MemoryStore
        from hushclaw.runtime.retention import RetentionExecutor

        with tempfile.TemporaryDirectory() as tmpdir:
            # Create an actual file to be deleted.
            artifact_file = Path(tmpdir) / "artifact.bin"
            artifact_file.write_bytes(b"data")
            self.assertTrue(artifact_file.exists())

            memory = MemoryStore(data_dir=Path(tmpdir) / "db")
            old_sec = int(time.time()) - 200 * 86400
            memory.conn.execute(
                "INSERT INTO artifacts (artifact_id, session_id, tool_name, storage_path, "
                "size_bytes, mime_type, summary, created) "
                "VALUES ('art-file', 'ses-file', 'write_file', ?, 4, 'application/octet-stream', '', ?)",
                (str(artifact_file), old_sec),
            )
            memory.conn.commit()
            ...
            self.assertFalse(artifact_file.exists(), "artifact file must be deleted from disk")
            remaining = [
                r[0] for r in memory.conn.execute("SELECT artifact_id FROM artifacts").fetchall()
            ]
            self.assertNotIn("art-file", remaining)
            memory.close()
        """RetentionExecutor must use events.artifact_id column to locate orphaned artifacts.
