...
const LANGS = {
  en: {
    ...
    desc_insights: "Curated principles and high-signal memory",
    ...
    sint_caldav_password_hint: "Use your provider's CalDAV credentials. iCloud requires an app-specific password.",
    sint_google_calendar_oauth_hint: "For Google Calendar, open Google Workspace below, enable calendar sync, and connect with OAuth 2.0. Disable any old Google CalDAV account in this form. Google passwords are not used.",
    sint_google_calendar_connect: "Set up Google Calendar (OAuth)",
    sint_google_calendar_sync: "Sync Google Calendar into Calendar",
    sint_google_calendar_sync_hint: "Read-only sync every 30 minutes, covering the past year and next two years, including recurring and all-day events. Save and authorize, then use Calendar → Sync to refresh immediately. Local edits are not sent to Google.",
    sint_google_calendar_custom_hint: "For automatic token refresh, use a Custom OAuth app with its Client ID and Client secret. Enable Google Calendar API, keep calendar.readonly in Scopes, and register this site's /oauth/app-connectors/google_workspace/callback URL. Managed tokens without client credentials may require reconnecting when they expire.",
    sint_google_calendar_docs: "Google authentication requirements",
  ...
  zh: {
    ...
    sint_caldav_password_hint: "请使用日历服务商提供的 CalDAV 凭据。iCloud 需要应用专用密码。",
    sint_google_calendar_oauth_hint: "Google Calendar 请从下方打开 Google Workspace，勾选日历同步并完成 OAuth 2.0 授权。请停用此表单中旧的 Google CalDAV 账号，无需填写 Google 密码。",
    sint_google_calendar_connect: "配置 Google Calendar（OAuth）",
    sint_google_calendar_sync: "将 Google 日历同步到 Calendar",
    sint_google_calendar_sync_hint: "每 30 分钟只读同步过去一年及未来两年的日程，包含重复和全天事件。保存并授权后，可在 Calendar 点击同步立即刷新。本地编辑不会写回 Google。",
    sint_google_calendar_custom_hint: "自动刷新令牌需要选择 Custom OAuth app 并填写对应的 Client ID 和 Client secret。请启用 Google Calendar API，在 Scopes 中保留 calendar.readonly，并将本站 /oauth/app-connectors/google_workspace/callback 地址注册为回调。Managed 模式缺少客户端凭据时，令牌过期后可能需要重新授权。",
    sint_google_calendar_docs: "Google 身份验证要求",
    sint_username:     "用户名",
    sint_cal_name:     "日历名称",
    sint_enabled:      "启用",
