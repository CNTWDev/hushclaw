# ghostclaw-skill-pptx

A bundled GhostClaw skill package for creating and editing PowerPoint presentations using [python-pptx](https://python-pptx.readthedocs.io/).

## Install via GhostClaw Web UI

1. Open GhostClaw Web UI → **Skills** tab → **Store**
2. Find **pptx-editor** and click **Install**
3. GhostClaw will automatically:
   - Clone this repo into your `skill_dir`
   - Run `pip install -r requirements.txt` in the GhostClaw Python environment
...
```bash
# 1. Clone into your skill directory (default: ~/.ghostclaw/skills/)
git clone https://github.com/CNTWDev/ghostclaw-skill-pptx ~/.ghostclaw/skills/ghostclaw-skill-pptx

# 2. Install dependencies into the same Python environment as GhostClaw
pip install -r ~/.ghostclaw/skills/ghostclaw-skill-pptx/requirements.txt

# 3. Restart ghostclaw serve — tools are auto-loaded on startup
ghostclaw serve
```
...
This repo is a reference implementation of the **GhostClaw Bundled Skill Package** format:
...
Standard pip requirements file. GhostClaw installs these into the **same Python environment** that runs `ghostclaw serve`, so your tools can import them immediately.
